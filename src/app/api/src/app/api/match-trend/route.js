...
export async function GET(request) {
  try {
    ...
    if (!bestFixture || bestFixture.score < 6) {
      return NextResponse.json(
        { ok: false, error: "Impossible d'associer ce match aux données API-Football." },
        { status: 404 }
      );
    }

    const fixtureId = bestFixture.fixture.fixture.id;
    const predictionsResult = await apiFootballFetch(`/predictions?fixture=${fixtureId}`);
    const prediction = predictionsResult?.response?.[0];

    if (!prediction?.predictions) {
      return NextResponse.json(
        { ok: false, error: "Tendance indisponible pour ce match." },
        { status: 404 }
      );
    }
